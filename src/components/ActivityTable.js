...
const ActivityTable = () => {
  ...
  const fetchData = () => {
    fetch('http://10.51.241.195:4002/api/cluster-data')
      .then(res => res.json())
      .then(data => {
        setClusterData(data);
        setLoading(false);
    ...
    clusterData.map((cluster, index) => {
      ...
      return (
        <tr key={index} style={{ borderBottom: '1px solid #2d3748' }}>
          ...
          <td className="p-3">
            <a
              href={`https://10.51.241.195:2224/ui/cluster/${cluster.name}/overview`}
              target="_blank"
              rel="noreferrer"
          ...
          <td className="p-3">
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {nodes.length === 0 ? (
                <span style={{ color: '#a0aec0', fontSize: '0.75rem' }}>No nodes</span>
              ) : (
                nodes.map(node => (
                  <span
                    key={node.name}
          ...
          <td className="p-3 text-gray-400" style={{ fontSize: '0.8rem' }}>
            {cluster.last_checked ? new Date(cluster.last_checked).toLocaleString() : '—'}
          </td>
        </tr>
      );
    })
